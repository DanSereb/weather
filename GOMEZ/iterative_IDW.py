...
calc_grid_weights = np.vectorize(grid_weights, excluded=['ROI'])


class IdwIterative():
    def __init__(self, ROI=0.0, max_iterations=1, tolerance=0.0):
        self.ROI = ROI
    ...
    def calc_grid_from_cmls(self):
        ...
        for xi in range(self.xgrid.shape[1]):
            for yi in range(self.ygrid.shape[0]):
                ...
                weights = calc_grid_weights(dist, self.ROI)
